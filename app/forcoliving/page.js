  ...
  return (
    <main className="max-w-[1260px] mx-auto p-4 height-[100%]">
      ...
      <section
        ...
        style={{ backgroundImage: "url(/images/bgcoliving.jpg" }}
      >
        <div className="absolute inset-0 bg-black opacity-50"></div>
        <div className="relative z-10 flex flex-col items-center justify-center h-full text-center text-white px-4">
          <h2 className="text-sm md:text-lg text-[#fff] font-bold">For Co Living</h2>
      ...
      <section className="mt-7 sm:mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 my-8">
        {data.filter((house)=> house.status == "For Co-Living" ).map((house)=>{
            return(
              ...
              <div
                className={` inset-0 flex  justify-center px-2 py-1 text-sm font-semibold text-white rounded ${
                  house.status === "For Co-Living" ? "bg-[#92400e]" : "bg-[#3b83f6d6]"
                }`}
              >
                {house.status}
              </div>
